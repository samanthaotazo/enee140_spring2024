...
int main(){
    ...
    //Solution 1: Shift and then insert
    /*//if m is a valid array index, insert integer x on position m of the array
    for (index = 7;index > m;index--){
        //Shift elements to the right...Ie a at index 7 takes the values of a at index 6
        a[index] = a[index - 1];
    }
    printf("Array after shifting elements from m to end to the right\n");
    printArray(a);
    //Insert x at element m
    a[m] = x;
